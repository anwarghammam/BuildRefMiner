...
SMELL_LABELS: dict[str, str] = {
    ...
    "MISSING_DEPENDENCY_VERSION": "Missing Dependency Version",
...
def _checks_for_build_type(build_type: str) -> list[tuple[str, CheckFn]]:
    if build_type == "ant":
        ...
        return [
            ...
            ("MISSING_DEPENDENCY_VERSION", ant_checks.check_missing_dependency_version),
            ("SUSPICIOUS_COMMENTS", ant_checks.check_suspicious_comments),
            ("DEPRECATED_DEPENDENCIES", ant_checks.check_deprecated_dependencies),
            ("OUTDATED_DEPENDENCIES", ant_checks.check_outdated_dependencies),
        ]
    ...
    if build_type == "gradle":
        ...
        return [
            ...
            ("MISSING_DEPENDENCY_VERSION", gradle_checks.check_missing_version_in_raw),
            ("SUSPICIOUS_COMMENTS", gradle_checks.check_suspicious_comment),
    ...
    if build_type == "maven":
        ...
        return [
            ...
            ("LACK_OF_ERROR_HANDLING", maven_checks.check_lack_of_error_handling),
            ("MISSING_DEPENDENCY_VERSION", maven_checks.check_missing_dependency_versions),
            ("SUSPICIOUS_COMMENTS", maven_checks.check_suspicious_comments),
...
class MaintainabilitySmellExtractor:
...
def extract_maintainability_smells(file_path: str, build_type: str | None = None) -> dict[str, Any]:
    return MaintainabilitySmellExtractor().detect_smells(file_path, build_type=build_type)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Extract maintainability smells using the secure_linter parser/checker modules."
    )
    ...
    try:
        result = extract_maintainability_smells(args.file_path, build_type=args.build_type)
    except Exception as exc:
